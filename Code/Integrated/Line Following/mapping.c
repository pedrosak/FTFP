...
void DetermineChoices(){
	...
	while (abs(encoderRight.read()) < 200){
		qtrrc.read(line);
		qtrrcRight.read(rightSensor);
		qtrrcLeft.read(leftSensor);
...
void Choose(){
	...
	for(int i = 0; i < currentNode; i++){
		if(curMap[currentNode][i][0] != 5){
			...
			//then turn around and go back to line following
			frame = (frame + 2)%4;
			curMap[currentNode][i][1] = 1;
			currentNode = i;
			return;
...
//creating internal map of course
 void mapping(){
	...
	if(mapLeft == true && mappedNodes[currentNode] == false){
		curMap[currentNode][nextOpenNode][0] = 1;
		curMap[nextOpenNode][currentNode][0] = 3;
		nextOpenNode++;
		mapLeft = false;
	}
	...
	if(mapRight == true && mappedNodes[currentNode] == false){
		curMap[currentNode][nextOpenNode][0] = 3;
		curMap[nextOpenNode][currentNode][0] = 1;
		nextOpenNode++;
		mapRight = false;
	}
	...
	if(mapStraight == true && mappedNodes[currentNode] == false){
		curMap[currentNode][nextOpenNode][0] = 0;
		curMap[nextOpenNode][currentNode][0] = 2;
		nextOpenNode++;
		mapStraight = false;
	}
